...
X = features_df.drop(columns=['window_mid_date', 'raw_return'])
y = features_df['raw_return']

split_index = int(0.8 * len(features_df))
X_train, X_test = X.iloc[:split_index], X.iloc[split_index:]
y_train_raw, y_test_raw = y.iloc[:split_index], y.iloc[split_index:]

def assign_class(r, quantiles):
    if r <= quantiles[0]:
...
quantiles = y_train_raw.quantile([0.2, 0.4, 0.6, 0.8]).values
y_train = y_train_raw.apply(lambda r: assign_class(r, quantiles))
y_test = y_test_raw.apply(lambda r: assign_class(r, quantiles))
