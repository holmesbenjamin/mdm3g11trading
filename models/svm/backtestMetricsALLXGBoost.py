...
def backtest_model(X, target_returns, clf, tss, mode):
    ...
    for fold, (train_idx, test_idx) in enumerate(tss.split(X)):
        ...
        y_train_orig = np.sign(target_returns.loc[X_train.index])
        y_train = ((y_train_orig + 1) // 2).astype(int)
...
def main():
    ...
    output_file = os.path.join(project_root, "xgb_backtest_results_summary_all.csv")
